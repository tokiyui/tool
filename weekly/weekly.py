...
def smooth_pressure(pressure, half_window=12, max_diff=1.0):
    ...
    corrected[diff > max_diff] = pressure[diff > max_diff] - max_diff
    corrected[diff < -max_diff] = pressure[diff < -max_diff] + max_diff
...
for model_id, model_name in models:
    ...
    for lat in latitudes:
        ...
        params = {
            "latitude": lat,
            "longitude": lon,
            "hourly": "pressure_msl,temperature_850hPa,geopotential_height_500hPa",
            "models": model_id,
            "timezone": "Asia/Tokyo",
            "forecast_days": 10
        }
        ...
        if "hourly" not in data:
            raise RuntimeError(f"{model_name}: API error\n{data}")
    ...
    for i in range(len(latitudes)):
        s = max(0, i - 4)
        e = min(len(latitudes), i + 5)

        pressure_smoothed[i] = np.mean(pressure_array[s:e], axis=0)
        temp850_smoothed[i] = np.mean(temp850_array[s:e], axis=0)
        z500_smoothed[i] = np.mean(z500_array[s:e], axis=0)

    # 必要なら追加平滑化
    # pressure_smoothed = smooth_5point(pressure_smoothed)
    ...
    for j in range(nx):
        for i in range(ny):
            ...
            if np.isclose(val, local_max, atol=0.05):
                highs.append((j, i, val))

            if np.isclose(val, local_min, atol=0.05):
                lows.append((j, i, val))

    print(f"{model_name}: H={len(highs)}  L={len(lows)}")
    ...
    outfile = f"time_latitude_{model_name}.png"
